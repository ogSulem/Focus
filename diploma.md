...
Для уточнения целевой аудитории и требований к функциональности сформирована карта потребностей ключевых пользовательских сегментов. В таблице 2 показаны основные цели, боли и проектные следствия, которые должны быть закрыты системой NeuroTrack.
...
Таблица 3. SWOT-анализ NeuroTrack
...
Для оценки эффективности рекомендаций в персональных системах продуктивности используются метрики, ориентированные не только на точность, но и на интерпретируемость и прикладную полезность. В таблице 5 приведен набор базовых метрик, применимых к rule-based модулю NeuroTrack.
...
Совокупность функциональных сценариев системы NeuroTrack, соответствующих сформированным требованиям, представлена на рисунке 1 в виде диаграммы вариантов использования.
...
- индекс по `userId` во всех пользовательских таблицах 26ля оптимизации выборок;
...
Скриншоты ключевых экранов (дашборд, аналитика, канбан, режим концентрации) представлены в приложении В на рисунках 14–В.4.

### 2.5.2 Потоки данных и управление состоянием интерфейса

Обмен данными организован через SWR с ревалидацией при фокусе окна, что обеспечивает актуальность метрик без перегрузки API. Общий поток запроса от пользователя до отображения данных представлен на рисунке 8.
...
Подробная декомпозиция тестируемых пользовательских сценариев приведена в приложении А (таблица 23), что обеспечивает трассируемость между требованиями раздела 1.4, проверкой в таблице 15 и эксплуатационными сценариями.
...
Дополнительные пользовательские потоки представлены в приложении Г (рисунки 18–Г.2), а матрица соответствия экранов и требований — в приложении Д (таблица 27).
...
Таблица 17. Сводная оценка нефункциональных требований
...
Дополнительно выполнена оценка устойчивости системы при типовых нагрузках пользователя: одновременная работа с дашбордом, аналитикой и обновлениями задач. Результаты нагрузочных замеров приведены в таблице 18.
...
Готовность к защите. По совокупности реализованных функциональных возможностей, полноте аналитического контура, уровню безопасности и качеству визуализации система соответствует целям и задачам выпускной квалификационной работы. Данный вывод подтверждается формализованной проверкой по таблицам 15 и 3.2.
...
Для дополнительного обоснования качества пользовательского опыта выполнена экспертная оценка интерфейса по эвристикам Нильсена. Итоговые результаты представлены в таблице 20.

Таблица 20. Оценка UX по эвристикам Нильсена
...
Таблица 21. Оценка трудоемкости и эффекта
...
В третьей главе выполнена комплексная оценка разработанной системы. Подтверждено выполнение всех 9 функциональных требований (таблица 15). Нефункциональные характеристики (производительность, изоляция данных, адаптивность, качество кода) формализованно оценены по пунктам НФТ-1…НФТ-5 (таблица 17). Проведен анализ практической применимости по типовым эксплуатационным сценариям. Определены перспективные направления дальнейшего развития системы. Для полноты верификации структура API и технологический контур вынесены в приложение Б: доменная структура backend API представлена в таблице 24, а технологический контур и версии компонентов — в таблице 25; это формирует замкнутый контур «требования — реализация — проверка».
...
|---|---|---|
| Анализ предметной области и существующих решений | Выявлены ограничения текущих систем и обоснована ниша NeuroTrack | Разделы 1.1–1.2, таблица 1 |
| Обзор методов аналитики и выбор подхода | Обоснован выбор explainable rule-based подхода | Раздел 1.3, таблица 4 |
| Формирование требований к системе | Сформированы 9 функциональных и 5 нефункциональных требований | Раздел 1.4, рисунок 1.1, таблица 15, таблица 17 |
| Проектирование архитектуры и модели данных | Спроектированы архитектура и модель данных из 8 сущностей | Разделы 2.2–2.3, рисунки 3 и 2.2, таблица 8 |
| Реализация backend/frontend компонентов | Реализован полный контур REST API и пользовательского интерфейса | Разделы 2.4–2.5, рисунки 5 и 2.4, таблица 9 |
...
| Оценка качества и практической применимости | Подтверждена функциональная полнота, НФТ и прикладная ценность | Раздел 3.1–3.4, таблицы 15–3.8 |
...
- диаграмма пользовательского цикла (рисунок 2.10), распределение UX-находок (рисунок 3.1) и скриншоты интерфейса (рисунки 14–В.4).
...
Итоговая полнота закрытия поставленных задач формально сведена в матрицу трассируемости (таблица 22), что дополнительно подтверждает логическую завершенность и внутреннюю согласованность ВКР.
...
С учетом требований методических указаний и результатов верификации работа соответствует критериям высокой итоговой оценки: тема актуальна и обоснована; исследование носит самостоятельный и завершенный характер; цель и задачи полностью раскрыты и формально трассированы (таблица 22); проектные решения подтверждены графическими материалами и таблицами; разработан функционирующий программный продукт с подтвержденной практической применимостью; выводы согласованы с поставленными задачами и результатами проверки; структура, нумерация и ссылочный аппарат согласованы с нормативными требованиями к ВКР.
...
Таблица 23. Функциональные сценарии системы NeuroTrack
...
На рисунках 14–В.4 представлены ключевые экраны пользовательского интерфейса NeuroTrack.
...
Таблица 27 отражает соответствие ключевых экранов интерфейса функциональным требованиям.

Таблица 27. Соответствие экранов и требований
...
Таблица 28. Глоссарий ключевых терминов
...
Таблица 29. Пример недельного отчета продуктивности
...
Таблица 30. Сводные показатели недели
