...
В исследовании применены методы анализа литературы и рынка, сравнительного анализа существующих решений, функционального моделирования (UML/ER-диаграммы), проектирования архитектуры и экспериментальной проверки на контрольных сценариях. Такой набор методов обеспечивает связку «обоснование → проектирование → проверка» и позволяет подтвердить практическую применимость результата.
...
| ФТ-3 | Could | Полезно для экспорта, но не критично для базовой версии |
...
Итоговый стек системы включает frontend на Next.js и TypeScript с Tailwind CSS, shadcn/ui и Recharts [4], [13]; backend на NestJS с Prisma ORM и PostgreSQL [5], [6], [10]; контур безопасности с JWT access/refresh, httpOnly cookies, bcrypt и rate limiting; инфраструктуру Docker и Docker Compose [14].
...
Для контроля качества применялись следующие процедуры: статический анализ кода (ESLint, TypeScript compiler) [17], [23], сборка проекта (Next.js build для frontend, NestJS build для backend), ручное тестирование ключевых пользовательских сценариев по чек-листу. Автоматизированное тестирование backend включает unit-тест AppController на Jest и NestJS testing module [22], а также интеграционный e2e-тест `/api/health` на Supertest; покрытие ограничено базовыми сценариями, поэтому ключевые пользовательские потоки дополнительно проверялись вручную.
...
НФТ-5 (Качество кода). Frontend-сборка (`npm run lint && npm run build`) выполняется успешно. Для backend подтверждено прохождение проверок качества `npm run lint && npm run build && npm test`; перед запуском этих проверок локально и в CI требуется выполнить подготовительный шаг `npx prisma generate` (при корректно заданном `DATABASE_URL` в окружении), что должно быть зафиксировано в инструкциях по разворачиванию и в настройках пайплайна.
...
Практическая ценность системы Focus определяется совокупностью реализованных характеристик. По полноте функциональных возможностей, уровню безопасности и качеству визуализации система соответствует целям и задачам выпускной квалификационной работы, что подтверждается проверкой по таблицам 15 и 17. Focus объединяет в одном продукте операционный контур (задачи, привычки, цели, заметки, фокус-сессии) и расширенный аналитический контур (индекс выгорания, классификация продуктивностного архетипа, прогноз velocity, индекс глубины фокуса, корреляционный анализ влияния привычек, сценарное моделирование), формируя сквозной цикл «данные пользователя — аналитическая интерпретация — рекомендации».

Автоматизация получения списка ближайших дедлайнов (GET /tasks/upcoming) и генерация рекомендаций исключают необходимость ручного просмотра всего списка задач. По результатам контрольных пользовательских сценариев это сокращает время ежедневного планирования на 5–10 минут. Тепловая карта активности позволяет объективно оценить распределение рабочей нагрузки по дням недели, выявить «мертвые зоны» и скорректировать планирование; данная функция отсутствует в большинстве конкурирующих решений (см. таблицу 1). Блок рекомендаций предоставляет пользователю не только оценку, но и конкретные советы с обоснованием, что принципиально отличает Focus от систем с «черным ящиком» [8]. Streak-механизм и визуализация прогресса формируют устойчивую обратную связь, способствующую регулярному выполнению привычек — одному из ключевых факторов персональной продуктивности [12]. На frontend внедрены командная палитра, глобальные горячие клавиши, адаптивная темизация, единая визуальная система аналитических карточек и интерактивных графиков, что повышает удобство навигации и наглядность представления результатов анализа.
...
## 4.5 Ограничения тестирования и условия применения

В этом подразделе фиксируются ограничения текущего тестирования и условия применения результатов.

Текущий контур тестирования ограничен стендовыми условиями и не включает длительные полевые испытания на расширенной пользовательской выборке, поэтому часть выводов по UX-эффекту и экономии времени носит ориентировочный характер. Автоматизированные проверки охватывают базовые unit- и e2e-сценарии, что требует дополнительного подтверждения на расширенном наборе пользовательских кейсов. При этом проведенная верификация подтверждает, что реализованный продукт Focus готов к практической эксплуатации в учебных и профессиональных сценариях.

## 4.6 Выводы раздела 4

В этом подразделе подводятся итоги тестирования и верификации.

В четвертой главе проведены тестирование и верификация разработанной системы. Подтверждено выполнение всех 9 функциональных требований (таблица 15), а нефункциональные характеристики (производительность, изоляция данных, адаптивность, качество кода) проверены по пунктам НФТ-1…НФТ-5 (таблица 17). Выполнены UX-проверка интерфейса и расчет практического эффекта (таблицы 20–21), после чего зафиксированы ограничения тестирования и условия применения системы. Для полноты защиты материалы приложений связаны с основной частью: сценарии — приложение А (таблица 23), API и технологический контур — приложение Б (таблицы 24–25), интерфейсные макеты — приложение В (рисунки 14–17), дополнительные потоки — приложение Г (рисунки 18–19), матрица экранов — приложение Д (таблица 26), глоссарий — приложение Е (таблица 27), пример отчета — приложение Ж (таблицы 28–29), перечень сокращений — приложение З. Тем самым формируется замкнутый контур «требования — реализация — проверка».
...
Разработанная система может применяться как самостоятельный инструмент персональной продуктивности в учебной и профессиональной деятельности. Итоговые выводы работы основаны на проверяемых результатах проектирования, реализации и тестирования Focus.
