...
На основе анализа предметной области, ограничений существующих решений и целей работы [15] сформирован следующий набор требований.
...
Архитектура системы построена по клиент-серверной модели с четкой сегментацией ответственности между слоями. Принципиальная схема системы включает следующие уровни:
...
Backend-часть реализует паттерн «модульная монолитная архитектура» (modular monolith). Каждый домен (tasks, habits, analytics, events, auth, users) оформлен как самостоятельный NestJS-модуль со следующей внутренней структурой:
...
Реляционная модель данных разрабатывалась из принципов минимальной избыточности, корректной нормализации (3НФ) и возможности эффективной аналитической агрегации.
...
Backend реализован на NestJS и организован в 7 функциональных модулей.
...
Оценка качества программного продукта проводилась в соответствии с принципами ISO/IEC 25010 (SQuaRE) [3], адаптированными к масштабу работы. Оценочные критерии сгруппированы в четыре категории.
...
НФТ-1 (Модульная архитектура). Backend организован в 7 изолированных NestJS-модулей: AuthModule, UsersModule, TasksModule, HabitsModule, AnalyticsModule, EventsModule, TelegramModule. Каждый модуль содержит явно определенные зависимости через декоратор @Module.
...
Временна я экономия. Автоматизация получения списка ближайших дедлайнов (GET /tasks/upcoming) и генерация рекомендаций исключают необходимость ручного просмотра всего списка задач. По оценке авторов, это сокращает время ежедневного планирования на 5–10 минут.
...
По задаче 4. Спроектирована клиент-серверная архитектура с разделением на 7 функциональных NestJS-модулей, разработана нормализованная реляционная модель данных на основе 5 сущностей.

По задаче 5. Реализованы backend REST API с 17 задокументированными endpoint-ами, frontend-приложение на Next.js с 10 экранами и системой визуализации данных.
...
15. Проектное задание NeuroTrack (prompt.md). GitHub: ogSulem/Focus, 2025. URL: https://github.com/ogSulem/Focus (дата обращения: 14.05.2026).
...
| Аналитика | /analytics | overview, weekly, monthly, heatmap, recommendations |
| События | — (внутренний) | Фиксация UserEvent при изменениях |
| Telegram | /telegram | webhook, команды |
...
| CSS-утилиты | Tailwind CSS | 3.x |
| UI-компоненты | shadcn/ui | актуальная |
| Графики | Recharts | 2.x |
| Backend-фреймворк | NestJS | 10.x |
| ORM | Prisma | 5.x |
