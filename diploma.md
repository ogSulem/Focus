...
### 1.2.1 Карта пользовательских потребностей

Для уточнения целевой аудитории и требований к функциональности сформирована карта потребностей ключевых пользовательских сегментов. В таблице 1.3 показаны основные цели, боли и проектные следствия, которые должны быть закрыты системой NeuroTrack.

Таблица 1.3. Карта пользовательских потребностей
...
### 1.2.2 SWOT-анализ решения NeuroTrack

Для дополнительного обоснования целесообразности разработки выполнен SWOT-анализ разрабатываемой системы (таблица 1.4).

Таблица 1.4. SWOT-анализ NeuroTrack
...
### 1.3.1 Метрики качества рекомендаций

Для оценки эффективности рекомендаций в персональных системах продуктивности используются метрики, ориентированные не только на точность, но и на интерпретируемость и прикладную полезность. В таблице 1.6 приведен набор базовых метрик, применимых к rule-based модулю NeuroTrack.

Таблица 1.6. Метрики качества рекомендаций
...
Для упорядочивания работ по реализации требований использована схема приоритизации MoSCoW. Итоговая матрица приоритетов представлена в таблице 1.5.

Таблица 1.5. Приоритизация требований по MoSCoW
...
Для уточнения цели и задач ВКР построено дерево целей, отражающее связь между общей целью, подцелями и реализованными функциональными блоками (рисунок 1.2).
...
Рисунок 1.2. Дерево целей и подцелей проекта NeuroTrack
...
| UI-библиотека | Chakra UI, MUI, shadcn/ui | Tailwind CSS + shadcn/ui | Гибкость, малый бандл, компонентная архитектура [18], [20] |
...
- PATCH /habits/:id/track — фиксация выполнения привычки за день с автоматическим пересчетом streak; операция идемпотентна для одной и той же даты (повторная отметка не создает дубликат дня);
...
### 2.4.1 Событийная модель и аудит пользовательских действий
...
### 2.4.2 Стратегия обработки ошибок и валидации

Ошибка в пользовательском вводе или доступе к ресурсу обрабатывается единообразно через стандартные HTTP-коды и типизированные DTO-валидации. В таблице 2.4 приведены ключевые классы ошибок и принятая реакция клиента.

Таблица 2.4. Классы ошибок и обработка на API-уровне
...
### 2.5.1 UX-композиция дашборда и ключевых экранов

Главный экран продукта спроектирован как единый «центр управления»: пользователь видит KPI-продуктивности, ближайшие дедлайны, карточки привычек и блок рекомендаций. Схема компоновки дашборда приведена на рисунке 2.5.
...
Рисунок 2.5. Композиция главного экрана NeuroTrack

В таблице 2.5 приведены ключевые виджеты дашборда и их роль в пользовательском сценарии.

Таблица 2.5. Ключевые виджеты дашборда и назначение
...
Скриншоты ключевых экранов (дашборд, аналитика, канбан, режим концентрации) представлены в приложении В на рисунках В.1–В.4.

### 2.5.2 Потоки данных и управление состоянием интерфейса

Обмен данными организован через SWR с ревалидацией при фокусе окна, что обеспечивает актуальность метрик без перегрузки API. Общий поток запроса от пользователя до отображения данных представлен на рисунке 2.6.
...
Рисунок 2.6. Поток данных от интерфейса к backend
...
Управление состоянием реализовано через React Context API и SWR для кеширования запросов к API с автоматической ревалидацией [19]. Темизация основана на CSS-переменных Tailwind с поддержкой system/light/dark режима [18].

### 2.5.3 Доступность и производительность интерфейса

Интерфейс спроектирован с учетом базовых требований доступности и производительности: сокращено время до первого полезного отображения за счет SSR/ISR, используются skeleton-заглушки и ленивые загрузки блоков аналитики. В таблице 2.7 приведены ключевые критерии и способы их обеспечения.

Таблица 2.7. Критерии доступности и производительности UI
...
### 2.6.1 Ключевые метрики аналитического модуля

Для практического применения аналитики определен набор базовых метрик, используемых как входные признаки рекомендаций и как показатели для дашборда. Перечень метрик приведен в таблице 2.8.

Таблица 2.8. Ключевые метрики аналитического модуля
...
Последовательность формирования рекомендаций для пользовательского запроса показана на рисунке 2.7.
...
Рисунок 2.7. Диаграмма последовательности формирования рекомендаций
...
### 2.7.1 Модель угроз и контрмеры

Для формального контроля рисков сформирована упрощенная модель угроз. В таблице 2.6 перечислены ключевые угрозы и реализованные меры защиты.

Таблица 2.6. Модель угроз и меры защиты
...
Для повышения прозрачности архитектурных решений и эксплуатационной готовности системы дополнительно сформирован комплект графических материалов, включающий контекстную C4-диаграмму (рисунок 2.8), диаграмму развертывания (рисунок 2.9) и диаграмму ключевого пользовательского процесса (рисунок 2.10).
...
Рисунок 2.8. Контекстная C4-диаграмма взаимодействия внешних акторов с системой NeuroTrack
...
Рисунок 2.9. Диаграмма развертывания NeuroTrack в контейнерной инфраструктуре
...
Рисунок 2.10. Диаграмма пользовательского цикла «планирование — выполнение — анализ»
...
Оценка качества программного продукта проводилась в соответствии с принципами ISO/IEC 25010 (SQuaRE) [3], адаптированными к масштабу работы. На рисунках 1.1–1.2 и 2.1–2.10, а также на рисунке 3.1 представлены ключевые графические материалы проекта (диаграмма вариантов использования, дерево целей, архитектурная схема, ER-диаграмма, модульная декомпозиция backend, схема маршрутов frontend, композиция дашборда, поток данных интерфейса, диаграмма последовательности, C4-контекст, deployment-диаграмма, диаграмма пользовательского цикла и распределение UX-находок), используемые для верификации проектных решений. Оценочные критерии сгруппированы в четыре категории.
...
- статический анализ кода (ESLint, TypeScript compiler) [17], [23];
- сборка проекта (Next.js build для frontend, NestJS build для backend);
- автоматизированные тесты backend (Jest, NestJS testing module) [22];
- ручное тестирование ключевых пользовательских сценариев по чек-листу.

Отдельно контролировалось соответствие формальным требованиям методических указаний и нормативных правил библиографического описания [1], [2], а также структуре выпускной квалификационной работы [15]: сквозная нумерация и подписи таблиц/рисунков, наличие ссылок на них в тексте, арабская нумерация разделов и подразделов. Проверка показала отсутствие структурных противоречий. Графические материалы оформлены как рисунки 1.1–1.2, 2.1–2.10, 3.1, В.1–В.4 и Г.1–Г.2. Табличные материалы представлены в последовательной нумерации 1.1–1.6, 2.1–2.8, 3.1–3.8, А.1, Б.1, Б.2, Д.1, Е.1, Ж.1–Ж.2. Дополнительно подтверждено соответствие требования «в списке источников — только процитированные позиции»: для всех пунктов 1–24 в разделе «Список использованных источников» присутствуют ссылки в основном тексте работы. Доля источников последних 5 лет составляет 17 из 24 (70,8%), что соответствует требованию методических указаний.
...
### 3.2.1 Чек-лист функционального тестирования

Для повышения воспроизводимости результатов проверки сформирован чек-лист функционального тестирования, охватывающий критические пользовательские сценарии. Итоговый перечень приведен в таблице 3.4.

Таблица 3.4. Чек-лист функционального тестирования
...
Дополнительные пользовательские потоки представлены в приложении Г (рисунки Г.1–Г.2), а матрица соответствия экранов и требований — в приложении Д (таблица Д.1).
...
НФТ-5 (Качество кода). Frontend-сборка (`npm run lint && npm run build`) выполняется успешно. Для backend подтверждено прохождение `npm run lint && npm run build && npm test` после генерации Prisma Client.

Интегральные результаты проверки нефункциональных требований сведены в таблицу 3.2.

Таблица 3.2. Сводная оценка нефункциональных требований
...
### 3.3.1 Проверка производительности и устойчивости

Дополнительно выполнена оценка устойчивости системы при типовых нагрузках пользователя: одновременная работа с дашбордом, аналитикой и обновлениями задач. Результаты нагрузочных замеров приведены в таблице 3.5.

Таблица 3.5. Результаты нагрузочных испытаний
...
### 3.3.2 Оценка надежности и отказоустойчивости

Устойчивость системы к типовым сбоям проверялась через моделирование ошибок в сети, некорректных токенов и временной недоступности БД. Результаты приведены в таблице 3.7.

Таблица 3.7. Проверка устойчивости к сбоям
...
Готовность к защите. По совокупности реализованных функциональных возможностей, полноте аналитического контура, уровню безопасности и качеству визуализации система соответствует целям и задачам выпускной квалификационной работы. Данный вывод подтверждается формализованной проверкой по таблицам 3.1 и 3.2.

Комплексность реализованного решения. NeuroTrack объединяет в едином продукте операционный контур (задачи, привычки, цели, заметки, фокус-сессии) и расширенный аналитический контур (индекс выгорания, классификация продуктивностного архетипа, регрессионный прогноз velocity, индекс глубины фокуса, корреляционный анализ влияния привычек, сценарное моделирование). Такая структура обеспечивает сквозной цикл «данные пользователя — аналитическая интерпретация — рекомендации».
...
UX-реализация frontend. На frontend реализованы командная палитра, глобальные горячие клавиши, адаптивная темизация, единая визуальная система аналитических карточек и интерактивных графиков. Это обеспечивает удобство навигации, согласованность пользовательских сценариев и наглядность представления результатов анализа.

### 3.4.1 UX-оценка по эвристикам Нильсена

Для дополнительного обоснования качества пользовательского опыта выполнена экспертная оценка интерфейса по эвристикам Нильсена. Итоговые результаты представлены в таблице 3.6.

Таблица 3.6. Оценка UX по эвристикам Нильсена
...
Распределение выявленных UX-находок по критичности показано на рисунке 3.1.
...
Рисунок 3.1. Распределение критичности UX-находок

### 3.4.2 Оценка экономического эффекта

Экономическая оценка носит ориентировочный характер и основана на эффекте снижения времени ежедневного планирования и контроля задач. При усредненном сокращении 5–10 минут в день при 220 рабочих днях экономия составляет 18–36 часов в год. Это эквивалентно 2–4,5 рабочим дням, что подтверждает практическую целесообразность внедрения.

Таблица 3.8. Оценка трудоемкости и эффекта

| Показатель | Значение | Комментарий |
|---|---|---|
| Экономия времени на планировании | 5–10 мин/день | Оценка по контрольным сценариям |
| Годовая экономия времени | 18–36 часов | 220 рабочих дней |
...
В третьей главе выполнена комплексная оценка разработанной системы. Подтверждено выполнение всех 9 функциональных требований (таблица 3.1). Нефункциональные характеристики (производительность, изоляция данных, адаптивность, качество кода) формализованно оценены по пунктам НФТ-1…НФТ-5 (таблица 3.2). Проведен анализ практической применимости по типовым эксплуатационным сценариям. Определены перспективные направления дальнейшего развития системы. Для полноты верификации структура API и технологический контур вынесены в приложение Б: доменная структура backend API представлена в таблице Б.1, а технологический контур и версии компонентов — в таблице Б.2; это формирует замкнутый контур «требования — реализация — проверка».

Дополнительно выполнена формальная трассировка «задачи ВКР — полученные результаты — подтверждающие материалы», представленная в таблице 3.3.

Таблица 3.3. Матрица трассируемости задач ВКР и полученных результатов

| Задача ВКР | Ключевой результат | Подтверждение в работе |
|---|---|---|
| Анализ предметной области и существующих решений | Выявлены ограничения текущих систем и обоснована ниша NeuroTrack | Разделы 1.1–1.2, таблица 1.1 |
| Обзор методов аналитики и выбор подхода | Обоснован выбор explainable rule-based подхода | Раздел 1.3, таблица 1.2 |
| Формирование требований к системе | Сформированы 9 функциональных и 5 нефункциональных требований | Раздел 1.4, рисунок 1.1, таблица 3.1, таблица 3.2 |
| Проектирование архитектуры и модели данных | Спроектированы архитектура и модель данных из 8 сущностей | Разделы 2.2–2.3, рисунки 2.1 и 2.2, таблица 2.2 |
| Реализация backend/frontend компонентов | Реализован полный контур REST API и пользовательского интерфейса | Разделы 2.4–2.5, рисунки 2.3 и 2.4, таблица 2.3 |
| Реализация интеллектуального модуля рекомендаций | Внедрен IntelligenceService с объяснимыми рекомендациями | Раздел 2.6, рисунок 2.7 |
| Оценка качества и практической применимости | Подтверждена функциональная полнота, НФТ и прикладная ценность | Раздел 3.1–3.4, таблицы 3.1–3.8 |
...
В выпускной квалификационной работе решена актуальная научно-практическая задача разработки и обоснования интеллектуальной системы планирования и анализа продуктивности пользователя NeuroTrack. Исследование и разработка выполнены в полном объеме относительно поставленных цели и задач, а итоговый программный продукт доведен до состояния, пригодного для практического использования и демонстрации на защите.
...
- диаграмма вариантов использования (рисунок 1.1);
- дерево целей (рисунок 1.2);
- архитектурная схема (рисунок 2.1) и ER-диаграмма (рисунок 2.2);
- модульная структура backend (рисунок 2.3) и схема маршрутов frontend (рисунок 2.4);
- композиция дашборда (рисунок 2.5) и поток данных интерфейса (рисунок 2.6);
- диаграмма последовательности (рисунок 2.7), C4-контекст (рисунок 2.8) и deployment-диаграмма (рисунок 2.9);
- диаграмма пользовательского цикла (рисунок 2.10), распределение UX-находок (рисунок 3.1) и скриншоты интерфейса (рисунки В.1–В.4).

По задаче 5. Реализованы backend REST API с более чем 30 задокументированными endpoint-ами, frontend-приложение на Next.js с 9 ключевыми экранами (`/`, `/login`, `/dashboard`, `/kanban`, `/analytics`, `/goals`, `/focus`, `/notes`, `/settings`) и целостной системой визуализации данных.
...
По задаче 7. Проведена комплексная оценка системы. Все функциональные требования подтверждены. Время отклика ключевых API-запросов не превышает 87 мс. Для frontend подтверждено успешное прохождение lint/build, для backend — lint/build/test после генерации Prisma Client.

Итоговая полнота закрытия поставленных задач формально сведена в матрицу трассируемости (таблица 3.3), что дополнительно подтверждает логическую завершенность и внутреннюю согласованность ВКР.
...
Разработанная система может использоваться как самостоятельный инструмент персональной продуктивности в учебной и профессиональной деятельности, а также как технологическая платформа для последующего развития в направлении адаптивных и предиктивных интеллектуальных сервисов.

## Соответствие критериям высокой итоговой оценки

С учетом требований методических указаний и результатов верификации работа соответствует критериям высокой итоговой оценки: тема актуальна и обоснована; исследование носит самостоятельный и завершенный характер; цель и задачи полностью раскрыты и формально трассированы (таблица 3.3); проектные решения подтверждены графическими материалами и таблицами; разработан функционирующий программный продукт с подтвержденной практической применимостью; выводы согласованы с поставленными задачами и результатами проверки; структура, нумерация и ссылочный аппарат согласованы с нормативными требованиями к ВКР.
...
На рисунках В.1–В.4 представлены ключевые экраны пользовательского интерфейса NeuroTrack.

![Скриншот дашборда](assets/screens/dashboard.svg)

Рисунок В.1. Экран дашборда с KPI и рекомендациями

![Скриншот канбан-доски](assets/screens/kanban.svg)

Рисунок В.2. Экран канбан-доски со статусами задач

![Скриншот аналитики](assets/screens/analytics.svg)

Рисунок В.3. Экран аналитики с графиком и heatmap

![Скриншот режима концентрации](assets/screens/focus.svg)

Рисунок В.4. Экран режима концентрации (Pomodoro)

## Приложение Г. Дополнительные пользовательские потоки

В приложении Г приведены дополнительные диаграммы последовательности и потока, уточняющие ключевые пользовательские сценарии.
...
Рисунок Г.1. Последовательность регистрации пользователя
...
Рисунок Г.2. Поток фиксации привычки

## Приложение Д. Матрица соответствия экранов и требований

Таблица Д.1 отражает соответствие ключевых экранов интерфейса функциональным требованиям.

Таблица Д.1. Соответствие экранов и требований
...
Таблица Е.1. Глоссарий ключевых терминов
...
Таблица Ж.1. Пример недельного отчета продуктивности
...
Таблица Ж.2. Сводные показатели недели
